...
function th_date($time){   // 19 ธันวาคม 2556 global $thai_day_arr,$thai_month_arr;
  $arr_month = config('global.th_month');
...
function th_date_short($time){   // 19  ธ.ค. 2556 date('Y-m-d');
  $arr_month = config('global.th_short_month');
  $d = strtotime($time);
  $th_date_short = date('j',$d).' '.$arr_month[date('n',$d)].' '.(date('Y',$d)+543);
  return $th_date_short;
}

function th_date_slash($time){   // 19/9/2563;
  $d = strtotime($time);
  $th_date_slash = date('j',$d).'/'.date('n',$d).'/'.(date('Y',$d)+543);
  return $th_date_slash;
}

function th_date_time_slash($time){   // 19/9/2563;
  $d = strtotime($time);
  $th_date_slash = date('j',$d).'/'.date('n',$d).'/'.(date('Y',$d)+543).'  '.date('H:i',$d) ;
  return $th_date_slash;
}
...
function th_date_scheduled_start($date){
  return substr($date,6,2).'/'.substr($date,4,2).'/'.(substr($date,0,4)+543);
}                               

function th_day($time){
  $arr_day = config('global.th_day');
  $d = strtotime($time);
  $th_day = $arr_day[date("w",$d)];
  return $th_day;
}

function en_day($time){
  $arr_day = array("sun","mon","tue","wed","thu","fri","sat");
  // $arr_day = config('global.en_day');
  $d = strtotime($time);
  $en_day = $arr_day[date("w",$d)];
  return $en_day;
}
