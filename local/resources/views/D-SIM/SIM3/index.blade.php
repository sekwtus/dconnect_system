          ...
          <div class="card-header text-center">
            <h4 class="font-weight-bold mb-0">
              ...
              {{-- <div class="card-actions">
                ...
                <a type="button" class="{{ Auth::user()->id_type_user == 1 ? '' : 'd-none' }}" onclick="setting_sim3('open-modal', {{ $item->topic_id }})" title="ตั้งค่า">
          ...
          <div class="card-body">
            ...
            <div class="row zoom60" >
              <div class="col-md-12" style="padding-right: 1px; padding-left: 1px;">
                <table border='1' class="table-bordered tbl-dconnect w-100" id="table_sim3_{{ $item->topic_id }}">
                  <thead>
                    <tr>
                      <th class="p-0">/</th>
                      <th class="p-0">Mon</th>
            ...
            <div class="row mt-2">
              <div class="col-md-12 px-0">
                <canvas id="chartjs-trend-sim3-{{ $item->topic_id }}" height="300"></canvas>
              </div>
  ...
  <div class="modal-dialog modal-xl">
    <form id="frm-sign-meeting">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <a href="{{ url('sim3/report_user_meeting') }}" class="btn btn-info">
...
<script type="">
  ...
  $.each(sim3_master, function (index, value) {
    ...
    // modal
    $(`#table_sim3_${value.topic_id} tbody`).on("click", "td", function () {  //call modal add data
      
      if ($(this).index() != 0) {
          var data_record = table[`table_sim3_${value.topic_id}`].row($(this).parents("tr")).data();
          ...
          $('#save_days').val($(`#table_sim3_${value.topic_id} thead th`).eq($(this).index()).text().trim());
          $('#save_date').val(today);
          $("input[name='txtTopicID']").val(value.id_sim3);

          $('.btn-save-data-sim3').attr('onclick', `save_data_sim3(${value.topic_id})`);
          ...
          $.ajax({
              ...
              success: function (data){ 
                console.log(data.SIM3_data_log[0]);
                if(data) {
                  ...
                  $('#ddlUser').val(data.SIM3_data_log[0].user_id);
                  $("#ddlUser").select2();
    ...
    if (todo === 'open-modal') {
      ...
      $.ajax({
        ...
        success: function (res) {
          // console.log(res.sim3_master);
          $('input[name="topic_eng"]').val(res.sim3_master.topic_eng);
          $('input[name="topic_detail"]').val(res.sim3_master.topic_detail);
          $('input[name="kpi"]').val(res.sim3_master.kpi);
          $('select[name="target"]').val(res.sim3_master.target);
          $('input[name="standard_rate"]').val(res.sim3_master.standard_rate);
          $('input[name="unit"]').val(res.sim3_master.unit);

          // color picker
          $(".colorpicker").asColorPicker();
          $('input[name="gauge_color1"]').asColorPicker('val',res.sim3_master.gauge_color1);
          $('input[name="gauge_color2"]').asColorPicker('val', res.sim3_master.gauge_color2);

          $('select[name="first_column"]').val(res.sim3_master.first_column);
          $('select[name="total_type_value"]').val(res.sim3_master.total_type_value);

          $('input[name="min_value"]').val(res.sim3_master.min_value);
          $('input[name="max_value"]').val(res.sim3_master.max_value);
          $('input[name="step_size"]').val(res.sim3_master.step_size);
          $('input[name="line_color"]').asColorPicker('val', res.sim3_master.line_color);

          $('select[name="show_line"]').val(res.sim3_master.show_line);
          $('input[name="score_decimals"]').val(res.sim3_master.score_decimals);
      ...
      $.ajax({
        type: "post",
        url: `{{ url('sim3/setting') }}`,
        data: $('#frm-setting-sim3').serialize(),
        success: function (res) {
          console.log(res);
          alert(res.msg);
          datatable_calling(res.sim3_master);
          $('#modal-setting-sim3').modal('hide');
          // ยังไม่destroy
          $("input[name='score']").trigger('touchspin.destroy');
  ...
  // save_data_sim3
  function save_data_sim3(topic_id) {
    // alert($('#ddlUser').val());
    ...
    $('.btn-save-data-sim3').attr('disabled', true);    
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        $('.btn-save-data-sim3').attr('disabled', false);
  ...
  function refresh_opject(topic_id) {
    ...
    var str = '';
    if (topic_id === 'all') {
      $.each(sim3_master, function (indexInArray, valueOfElement) { 
          datatable_calling(valueOfElement);
          str = str +','+ valueOfElement.topic_id;
      });
      console.log('fn-[refresh_opject-all]-topic'+str);
    } else {
      var value_sim3_master;
      $.each(sim3_master, function (indexInArray, valueOfElement) { 
        if(valueOfElement.topic_id == topic_id){
          value_sim3_master = valueOfElement;
        }
      });
        
      datatable_calling(value_sim3_master);
  ...
  function datatable_calling(value) {
    ...
    // table
    table[`table_sim3_${value.topic_id}`] = $(`#table_sim3_${value.topic_id}`).DataTable( {  //datatable
      destroy: true,
      ...
      columnDefs: [
        ...
        },   
        {
            ...
            createdCell: function (td, celldata, row, numrow, col){
              ...
              // $(td).html((row.score_Tue ? parseInt(row.score_Tue).toFixed( value.score_decimals ) : null ));
  ...
  function trend_chart(value, arr_total) {
    ...
    arr_trend_chart[`trend_chart${value.topic_id}`] = new Chart(document.getElementById(`chartjs-trend-sim3-${value.topic_id}`), {
      type: "line",
      data: {
        labels: arr_period,
        datasets: [
...
<script>
              ...
              return '<button type="button" class="btn waves-effect waves-light btn-info"\
                      ...
                      \
                       <button type="button" class="btn waves-effect waves-light btn-success"\
                        onclick="send_mail( `'+row.sim3_log_id+'`,`'+row.user_id+'` )">\
                        <i class="mdi mdi-email"></i>\
                      </button>\
                      ';
            },
          ...
          $.ajax({
            ...
            data: {sim3_log_id:sim3_log_id},
              ...
              success: function(result) {
                  table_pdaca_sim3.ajax.reload();
                  refresh_opject(sim3_topic_id); //fixedก่อน เพราะค่าวนเป็น index แต่save topic เป็น id
                  alert('ลบสำเร็จ');
              }
          });
      }
  ...
  function send_mail(sim3_log_id,user_id){
      $('.modal-send-mail').modal('show');
      // if (confirm('ยืนยันการส่งอีเมล)) {
          $.ajax({
            url: '{{ url('send-email-sim3x') }}',
            data: {sim3_log_id:sim3_log_id, user_id:user_id},
              type: 'get',
              success: function(result) {
  ...
  function set_start_meeting(){
    ...
    $.ajax({
            ...
            success: function (meeting_time) {
              ...
              $('#showHeader').html('SIM 3 ({{ th_date_short(date("Y-m-d")) }})');
...
<script>
  function datatable_information_sharing(){
      ...
      ajax: {
            url: '{{ url('D-SIM/sim3/get_table_information_sharing') }}',
            complete: function(){
            },
            error: function(){
              table_pdaca_sim3.ajax.reload();
      ...
      columnDefs: [
          ...
          },
          {
            targets: 3,
            render(data,type,row){
              return '<button type="button" onclick="delete_sim3_info_sharing('+data+')" class="btn waves-effect waves-light btn-danger">\
                       <i class="mdi mdi-delete"></i>\
                      </button>';
            },
          },
  ...
  function swap_table(){    
    if ($('.text-swap_table').text() === 'Information sharing' ) {
      ...
      datatable_pdca();
      table_pdaca_sim3.ajax.reload(); // แสดงตาราง
  ...
  function modal_info_sharing(even){
    if (even === 'show') {
        ...
        $.ajax({
          type: 'post',
          dataType: 'JSON',
          url: `{{ url('D-SIM/sim3/save_information_sharing') }}`,
          data: form_data,
          contentType: false,
          processData: false,
          beforeSend: function(){
  ...
  function delete_sim3_info_sharing(id){
      if (confirm('ยืนยันการลบ')) {
          $.ajax({
            url: '{{ url('D-SIM/sim3/delete_sim3_information_sharing') }}',
            data: {id:id},
              type: 'delete',
              success: function(result) {
                  table_information_sharing.ajax.reload();
