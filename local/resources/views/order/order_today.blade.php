    ...
    function datatable(date_spec){
        var table =  $('#order_table').DataTable( {
            ...
            columns: [
                ...
                { data: 'GroupNo', className:'text-center ' },
                { data: 'production_order', className:'text-center' },
            ...
            columnDefs: [
                {
                    ...
                    render(data,type,row){
                        ...
                        if ('{{ Auth::user()->id_type_user }}' == 3 ) {
                            return `<a href="report/${data}" class="btn btn-outline-info font-weight-bold">
                                    REPORT
                                </a>`;
                            ...
                            return `<a href="report/${data}" class="btn btn-outline-info font-weight-bold">
                                ...
                                <a href="printer_monitor/${data}" class="btn btn-outline-success font-weight-bold" title="เลือกเอกสาร">
                                    Paperless
                                </a>`;

                        } else {
                            return `<a href="printer_monitor/${data}" class="btn btn-outline-success font-weight-bold" title="เลือกเอกสาร">
                                    Paperless
                                </a>`;
                        }
...
<script>
    ...
    var day_range = $('#day_range').data('daterangepicker').startDate;

    date_start = moment(day_range).format('DD/MM/YYYY');
    date_end = moment(day_range).add(7,'d').format('DD/MM/YYYY');
